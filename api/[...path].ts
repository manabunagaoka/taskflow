...
app.get("/api/members", async (_req, res) => {
  const result = await db.select().from(schema.members).orderBy(asc(schema.members.id));
  res.json(result);
});
...
app.get("/api/projects", async (_req, res) => {
  const result = await db.select().from(schema.projects).orderBy(asc(schema.projects.id));
  res.json(result);
});
...
app.get("/api/tasks", async (_req, res) => {
  const result = await db.select().from(schema.tasks).orderBy(asc(schema.tasks.order));
  res.json(result);
});
